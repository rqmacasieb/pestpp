...
vector<double> ParetoObjectives::get_euclidean_distance(map<string, double> first, map<string, double> second, map<string, double> objrange)
{
	vector<double> euclidean_dist{ 0, 0 };

	for (auto obj : *obs_obj_names_ptr)
		euclidean_dist.at(0) += pow((first[obj] - second[obj]) / objrange[obj], 2);

	if (prob_pareto)
	{
		for (auto obj : *obs_obj_names_ptr)
		{
			euclidean_dist.at(0) += (pow(first[obj + "_SD"], 2) + pow(second[obj + "_SD"], 2)) / pow(objrange[obj], 2);
			euclidean_dist.at(1) += 4 * pow((first[obj] - second[obj])/objrange[obj], 2) 
				* (pow(first[obj + "_SD"], 2) + pow(second[obj + "_SD"], 2)/ pow(objrange[obj], 2)) 
				+ 2 * pow((pow(first[obj + "_SD"], 2) + pow(second[obj + "_SD"], 2)) / pow(objrange[obj], 2), 2);
		}
...
pair<map<string, double>, map<string, double>> ParetoObjectives::get_euclidean_crowding_distance(vector<string>& members, map<string, map<string, double>>& _member_struct)
{
	...
	map<string, double> crowd_distance_map, var_distance_map, euclidean_fitness_map, obj_sd_map;
	string m = members[0];
	vector<string> obj_names;
	//for (auto obj_map : _member_struct[m])
	//{
	...
	for (auto obj_map : obj_member_map)
	{
		...
		if (crowd_sorted.size() == 3)
		{
			...
			if (eucd_prev_it.at(0) > crowd_distance_map[it->first])
			{
				...
				euclidean_fitness_map[it->first] = get_euclidean_fitness(eucd_prev_it.at(0), eucd_prev_it.at(1));
			}
			...
			if (eucd_prev_it.at(0) > crowd_distance_map[it->first])
			{
				...
				euclidean_fitness_map[it->first] = get_euclidean_fitness(eucd_it_next.at(0), eucd_it_next.at(1));
			}
		}
		else if (crowd_sorted.size() > 3)
		{
...
vector<string> MOEA::get_pso_gbest_solutions(int num_reals, ParameterEnsemble& _dp, ObservationEnsemble& _op)
{
	...
	////fitness calc by prob of nondom
