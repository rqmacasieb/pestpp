...
map<string, double> ParetoObjectives::get_mopso_fitness(vector<string> members, map<string, map<string, double>>& _member_struct)
{
	...
	if (prob_pareto)
	{
		...
		sortedset crowd_sorted(cluster_crowding.begin(), cluster_crowding.end(), compFunctor);
...
void ParetoObjectives::prep_pareto_summary_file(string summary_tag)
{
	...
	if (prob_pareto)
	{
		...
		sum << ",nsga2_front,nsga2_crowding_distance,prob_nondom,spea2_unconstrained_fitness,spea2_constrained_fitness,is_feasible,feasible_distance" << endl;
	}
	else
...
map<string, double> ParetoObjectives::get_cluster_crowding_fitness(vector<string>& members, map<string, map<string, double>>& _member_struct)
{
	...
	map <string, double> d_ij, min_dij;
	for (auto i : expdist_lookup)
	{
		d_ij = i.second;
		double min = 1E+30;
		for (auto j : d_ij)
	...
	double maxmin_dij = -1E+30;
	for (auto d : min_dij)
	{
		if (d.second > maxmin_dij)
			maxmin_dij = d.second;
	}
	...
	for (auto m : members)
	{
		...
		*/
		...
		fit_map[m] = pnd /*pow(pnd,pow(nn, -1))*/;
