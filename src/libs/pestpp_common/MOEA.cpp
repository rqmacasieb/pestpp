...
map<string, map<string, double>> ParetoObjectives::get_bgo_ensemble_struct(map<string, map<string, double>> _member_struct)
{
	performance_log->log_event("computing AQF of the current ensemble");
	bgo_aqf_map.clear();
	map<string, map<string,double>> bgo_struct;
	for (auto o: *obj_names_ptr)
	{
		map <string, double> bgo_member;
		for (auto m : _member_struct)
		{
			bgo_member["AQF"] = -get_ei(_member_struct[m.first], o, c_opt);
			bgo_struct[m.first] = bgo_member;
			bgo_aqf_map[m.first] = bgo_member["AQF"];
		}
	}
...
bool ParetoObjectives::compare_two_bgo(string& first, string& second)
{
	if (bgo_aqf_map.at(first) < bgo_aqf_map.at(second))
		return true;
	if (bgo_aqf_map.at(first) == bgo_aqf_map.at(second))
	{
		if (enbgo_fitness_map.at(first) > enbgo_fitness_map.at(second))
			return true;
		else
			return false;
	}
...
//map<string, double> ParetoObjectives::get_decision_space_dist(vector<string> nondom, vector<string> dom, ObservationEnsemble& op, ParameterEnsemble& dp)
//{
//	map<string, map<string, double>> _member_struct = get_member_struct(op, dp);
//	return get_decision_space_dist(nondom, dom, _member_struct);
//}
//
//map<string, double> ParetoObjectives::get_decision_space_dist(vector<string> nondom, vector<string> dom, map<string, map<string, double>>& _member_struct)
//{
//	return;
//}
...
void ParetoObjectives::update(ObservationEnsemble& op, ParameterEnsemble& dp, Constraints* constraints_ptr)
{
	...
	//setup container for ei values of the members
	/*if (bgo)
		bgo_ensemble_struct = get_bgo_ensemble_struct(member_struct);*/
...
void ParetoObjectives::write_bgo_ensemble_summary(string& sum_tag, int generation, ObservationEnsemble& op, ParameterEnsemble& dp, Constraints* constr_ptr)
{
	...
	for (auto member : op.get_real_names())
	{
		...
		sum << "," << bgo_member_repo_map[member];
		sum << "," << -bgo_aqf_map[member];
		sum << "," << decspace_dist_map[member];
		sum << "," << enbgo_fitness_map[member];
...
void ParetoObjectives::prep_bgo_ensemble_summary_file(string summary_tag)
{
	...
	sum << ",importance_rank,AQF,distance_to_maxAQF,importance,is_feasible,feasible_distance" << endl;
}

void ParetoObjectives::write_training_summary(int generation, int inner_iter, ParameterEnsemble& dt, ObservationEnsemble& ot, string sum_tag)
{
	//save a summary of all the training dataset
	if (!file_manager.check_ofile_tag_exists(sum_tag))
		file_manager.open_ofile_ext(sum_tag);
	ofstream& sum = file_manager.get_ofstream(sum_tag);

	Eigen::RowVectorXd obs_vals, dv_vals;
	for (auto member : ot.get_real_names())
	{
		sum << generation << "," << inner_iter << "," << pest_utils::lower_cp(member);
	
		for (int i = 0; i < ot.shape().second; i++)
		{
			obs_vals = ot.get_eigen(vector<string>{member}, vector<string>());
			sum << "," << obs_vals[i];
		}
		
		for (int i = 0; i < dt.shape().second; i++)
		{
			dv_vals = dt.get_eigen(vector<string>{member}, vector<string>());;
			sum << "," << dv_vals[i];
		}
		sum << endl;
	}

	cout << "...wrote all-generation training dataset to " << file_manager.get_base_filename() << "." << sum_tag << endl;
	file_manager.rec_ofstream() << "...wrote all-generation training dataset to " << file_manager.get_base_filename() << "." << sum_tag << endl;

}

void ParetoObjectives::prep_bgo_training_summary_file(string summary_tag)
{
	file_manager.open_ofile_ext(summary_tag);
	ofstream& sum = file_manager.get_ofstream(summary_tag);
	vector<string> obsnames = pest_scenario.get_ctl_ordered_nz_obs_names();
	
	sum << "generation,inner_iter,member";
	for (auto obs : obsnames)
		sum << "," << pest_utils::lower_cp(obs);
	/*for (auto obj : *pi_obj_names_ptr)
		sum << "," << pest_utils::lower_cp(obj);
	for (auto objsd : *obs_obj_sd_names_ptr)
		sum << "," << pest_utils::lower_cp(objsd);*/

	vector<string> parnames = pest_scenario.get_ctl_ordered_par_names();
	for (auto dv : parnames)
		sum << "," << pest_utils::lower_cp(dv);
	sum << endl;
}
	...
	ParameterEnsemble& dp, ObservationEnsemble& op, Constraints* constraints_ptr, double lambda)
{
	...
	for (auto m1 : aqf_vals)
	{
		count = 0;
		for (auto m2 : aqf_vals)
		{
			if (m1.second <= m2.second)
				count++;
		}
		aqf_pct[m1.first] = count / numreals;
	...
	while (selected < numreals)
	{
		...
		{
			if (find(selected_members.begin(), selected_members.end(), en.first) != selected_members.end()) //check if the member has been added to the repo
				continue;

			if (_ensemble_struct[en.first]["AQF"] <= mx)
			{
				mx = _ensemble_struct[en.first]["AQF"];
				mxid = en.first;
			}
		}
		...
		selected_members.push_back(candidate);
		enbgo_fitness_map[candidate] = mx_fit;
		decspace_dist_map[candidate] = dist_pct[candidate];
		selected++;
		repo_map[selected] = candidate;
	...
	vector<pair<string, double>> en_vec;
	for (auto en : enbgo_fitness_map)
		en_vec.push_back(en);

	std::sort(en_vec.begin(), en_vec.end(),compFunctor);
	reverse(en_vec.begin(), en_vec.end());

	vector<string> ordered_ensemble;
	map<int, string> ensemble_map;
	int rank = 0;
	bgo_member_repo_map.clear();
	for (auto en : en_vec)
	{
		rank++;
		ensemble_map[rank] = en.first; 
		bgo_member_repo_map[en.first] = rank;
	}*/
...
{
	vector<string> objsd = *obs_obj_sd_names_ptr;
	...
	
{
	...
	ot.set_pest_scenario_ptr(&pest_scenario);
}
...
vector<int> MOEA::run_surrogate(ParameterEnsemble& _dp, ObservationEnsemble& _op)
{
	ofstream& frec = file_manager.rec_ofstream();
	stringstream ss;
	...
	{
		stringstream ss;
		ss << "run_surrogate() error queueing runs: " << e.what();
		throw_moea_error(ss.str());
	}
	catch (...)
	{
		throw_moea_error(string("run_surrogate() error queueing runs"));
	}
	performance_log->log_event("making surrogate runs");
	try
	{
	...
	performance_log->log_event("processing surrogate runs");
...
void MOEA::initialize()
{
	...
	else if (env == "NSGA_BGO")
	{
		envtype = MouEnvType::NSGA;
		bgo = true;
		objectives.set_bgo_mode(bgo);
		message(1, "using 'nsga2_bgo' env selector");
	}
	else if (env == "SPEA")
	{
	...
	else
	{
		...
		for (auto obj_name : passed_obj_names)
		{
			...
			else if (oset.find(obj_name) != oset.end())
			{
				...
				if (prob_pareto || bgo) keep_obs_sd.push_back(obj_name + "_SD");
			}
			...
			else
			{
				...
				if (prob_pareto || bgo) keep_pi_sd.push_back(obj_name + "_SD");
			}
		}
		if (err_sense.size() > 0)
		{
	...
	//if (bgo)
	//{
	//	message(1, "performing initial ensemble sorting");
	//	objectives.set_pointers(obj_names, obs_obj_names, obs_obj_sd_names, pi_obj_names, pi_obj_sd_names, obj_dir_mult);
	//	vector<string> keep;
	//	
	//	//EnBGO currently works for NSGA only
	//	
	//	get_current_true_solution();

	//	DomPair ensemble = objectives.get_bgo_ensemble(iter, op, dp, &constraints, true, BGO_POP_SUM_TAG);

	//	//drop any duplicates
	//	keep.clear();
	//	for (auto en : ensemble.first)
	//	{
	//		keep.push_back(en);
	//	}
	//	for (auto en : ensemble.second)
	//	{
	//		keep.push_back(en);
	//	}
	//	if (keep.size() == 0)
	...
	//		op.get_eigen(ensemble.first, vector<string>()), ensemble.first, op.get_var_names());
	//	dp_archive = ParameterEnsemble(&pest_scenario, &rand_gen,
	//		dp.get_eigen(ensemble.first, vector<string>()), ensemble.first, dp.get_var_names());

	//	ss.str("");
	//	ss << "initialized archives with " << ensemble.first.size() << " ensemble members";
	//	message(2, ss.str());
...
void MOEA::save_training_dataset(ParameterEnsemble& _dp, ObservationEnsemble& _op, string tag)
{
	//save current training dataset with highest EI member added assuming it is true
	stringstream ss;
	string fname;
	...
	ss.str("");
	_op.reset_org_real_names();
	ss << file_manager.get_base_filename();
	if (tag.size() > 0)
	{
		ss << "." << tag;
	}
	ss << "." << training_obs_file_tag << ".csv";
	_op.to_csv(ss.str());
...
void MOEA::get_current_true_solution()
{
	stringstream ss;
	map<string, map<string, double>> curr_opt;
	vector<string> tokens;

	//for now, the true solution need to be obtained from the archive of the previous outer repo where PBM was evaluated and nondom solution (curr opt) sorted out
	string outer_repo_obs_filename = pest_scenario.get_pestpp_options().get_mou_outer_repo_obs_file();
	if (outer_repo_obs_filename != "")
	{
		message(1, "loading outer repository obs from csv file", outer_repo_obs_filename);
