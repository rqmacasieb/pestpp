...
Eigen::MatrixXd GPutils::covar(const Eigen::MatrixXd& X1, const Eigen::MatrixXd& X2, double d)
	{
		Eigen::MatrixXd D = distance(X1, X2);
		return (-D / d).array().exp().matrix();
	}

Eigen::MatrixXd GPutils::covar_symm(const Eigen::MatrixXd& X, double d, double g)
	{
...
vector<int> GPutils::closest_indices(int start, const Eigen::RowVectorXd& xref,
	...
	const double* dab, const double* gab, int verb)
{
	for (int i = 0; i < 100; i++)
	{
		...
		if (fabs(d - d_old) < GPutils::sqrt_eps() && fabs(g - g_old) < GPutils::sqrt_eps())
			break;
	}
}
...
void GP::predict_lite(const Eigen::MatrixXd& Xref, Eigen::VectorXd& mean, Eigen::VectorXd& s2) const
{
	...
	Eigen::MatrixXd k = GPutils::covar(Xref, X, d);
